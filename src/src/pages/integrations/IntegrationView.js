...
import { get, del } from '../../redux/slices/integration';
// routes
import { PATH_DASHBOARD } from '../../routes/paths';
// components
import StandardPage from '../../layouts/StandardPage';
import { IntegrationProfile, IntegrationCover } from '../../components/integrations';
...
export default function IntegrationView() {
  ...
  const { current } = useSelector((state) => state.integration);
  const { id } = useParams();
  ...
  useEffect(() => {
    if (current.status === 'success') {
      ...
      navigate(PATH_DASHBOARD.integrations.list);
    }
  ...
  const ViewActions = () => (
    <Stack direction="row" spacing={1} sx={{ pt: 3 }}>
      <Button size="medium" variant="contained" component={RouterLink} to={`${PATH_DASHBOARD.integrations.list}`}>
        Back
      </Button>
    ...
    >
      {current.hasData && (
        <>
          ...
          >
            <IntegrationCover integration={current.response.currentVersion} />
